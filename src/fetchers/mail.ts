...
export function fetchMailData(): string {
    ...
    `;
		...
		if (!result) return "No mail data found";
		...
		if (parts.length < 1) {
			...
			return "Mail script result in unexpected format";
		}
		...
		if (Number.isNaN(unreadCount)) {
			...
			return "Invalid unread count from mail script";
		}
		let output = `Mail: ${unreadCount} unread`;

		if (subjectsStr) {
			// Remove trailing delimiter and trim
			const subjects = subjectsStr.replace(
				new RegExp(`\\s*${SUBJECT_DELIMITER}\\s*$`),
				"",
			);
		...
		console.error(
			"Mail fetch error (ensure Accessibility/Automation permissions):",
			err,
		);
		return "Mail fetch error";
	}
}
